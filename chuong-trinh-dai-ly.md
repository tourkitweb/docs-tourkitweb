# CHƯƠNG TRÌNH ĐẠI LÝ

### 1. Chương trình hoạt động thế nào

Đại lý là một loại tài khoản riêng trên website. Bạn bán dịch vụ của sàn (tour, khách sạn, du thuyền, sự kiện, vé máy bay…) cho khách của mình và được sàn trả lại một phần tiền. Có hai chế độ vận hành, sàn chọn một, và hai chế độ trả tiền theo hai cách khác hẳn nhau.

**Chế độ ĐƠN CẤP**

* Bạn đăng nhập rồi tự đặt đơn → giá GIẢM NGAY theo mức của hạng bạn đang mang. Bạn thu tiền khách theo giá niêm yết, phần chênh là của bạn.
* Khách do bạn giới thiệu tự đặt → bạn nhận HOA HỒNG vào ví khi đơn hoàn tất.
* Không có tuyến dưới. Menu "Tuyến dưới của tôi" không hiện.
* Mọi đại lý đều mang một hạng.

**Chế độ PHÂN CẤP (đang chạy)**

* **Không còn chiết khấu trực tiếp** — mọi đơn hiện giá gốc như khách thường. Toàn bộ quyền lợi chuyển thành hoa hồng vào ví.
* Có cây tuyến: bạn tuyển được đại lý cấp dưới, và ăn một phần trên đơn họ bán.
* Chỉ đại lý Cấp 1 mang hạng. Cấp 2 trở xuống KHÔNG có hạng — quyền lợi của cả nhánh do hạng của Cấp 1 quyết định.

_**Đừng nhầm hai chế độ:**_ Sàn bật hay tắt phân cấp là quyết định của sàn, không phải của từng đại lý. Ngay khi bật phân cấp, chiết khấu trực tiếp bị cắt cho TẤT CẢ đại lý — kể cả Cấp 1 — và tiền chuyển hết sang đường hoa hồng vào ví. Nếu hôm nay bạn thấy giá không còn giảm nữa thì đó là lý do, không phải lỗi.

#### Bốn bước từ lúc bắt đầu tới lúc có tiền

1. **Có tài khoản đại lý.** Tự đăng ký hoặc được quản trị viên gán.
2. **Đưa khách vào tuyến của bạn.** Bằng link giới thiệu, hoặc để khách gõ số điện thoại của bạn vào ô "Mã giới thiệu" ở bước thanh toán.
3. **Khách đặt và thanh toán.** Đơn chuyển sang trạng thái đã thanh toán / đã xác nhận / đã hoàn tất thì hệ thống tự ghi sổ hoa hồng.
4. **Tiền vào ví.** Sàn giữ 5 ngày kể từ ngày kết thúc dịch vụ, sau đó tự cộng vào ví.

### 2. Trở thành đại lý

#### Cách 1 — Tự đăng ký

Trang đăng ký đại lý là một trang RIÊNG, khác trang đăng ký khách thường: [https://demo.tourkitweb.vn/dang-ky/dai-ly](https://demo.tourkitweb.vn/dang-ky/dai-ly)

| Thông tin           | Yêu cầu                                                                                                                                    |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Họ và tên           | Bắt buộc.                                                                                                                                  |
| Email               | Bắt buộc, chưa ai dùng. Đây là email nhận thư xác minh — điền email thật của bạn, không mượn của người khác.                               |
| Số điện thoại       | Bắt buộc, chưa ai dùng. ĐÂY CHÍNH LÀ MÃ GIỚI THIỆU của bạn sau này — khách sẽ gõ số này ở bước thanh toán.                                 |
| Mật khẩu            | Tối thiểu 8 ký tự, phải có chữ in hoa, chữ thường, số và ký tự đặc biệt.                                                                   |
| Mã giới thiệu       | Số điện thoại của đại lý đã giới thiệu bạn. **Website này đang yêu cầu BẮT BUỘC phải có mã mới đăng ký được.**                             |
| Thông tin ngân hàng | Tên ngân hàng (có gợi ý sẵn), số tài khoản, chủ tài khoản. Không bắt buộc lúc đăng ký nhưng nên điền — sàn dùng thông tin này khi chi trả. |

**Đăng ký xong thì sao?**

* **Có mã giới thiệu hợp lệ** website này: kích hoạt ngay → tài khoản đại lý được bật NGAY, không phải chờ, và bạn trở thành cấp dưới trực tiếp của người giới thiệu.
* **Không có mã giới thiệu** → hồ sơ vào hàng chờ, quản trị viên duyệt tay. Duyệt xong bạn là đại lý Cấp 1 độc lập và được gán một hạng.

Đăng ký báo thành công nhưng chưa dùng được ngay Kể cả khi bạn gõ đúng mã, vẫn có mấy trường hợp hệ thống lặng lẽ đưa hồ sơ về hàng chờ thay vì kích hoạt: sàn tắt chế độ tự duyệt, nhánh của người giới thiệu đã chạm giới hạn số cấp, hoặc người giới thiệu vừa bị khoá. Bạn không sửa được mấy tình huống này — cứ chờ quản trị viên duyệt, hoặc liên hệ người đã giới thiệu bạn.

#### Cách 2 — Quản trị viên gán trực tiếp

Bạn đã có tài khoản khách thường trên website, quản trị viên gán hạng đại lý cho tài khoản đó. Bạn trở thành đại lý Cấp 1. Không cần đăng ký lại, giữ nguyên email và mật khẩu cũ.

Một tài khoản chỉ mang MỘT vài Đại lý, nhà cung cấp (vendor) và cộng tác viên affiliate là ba loại tài khoản LOẠI TRỪ NHAU. Tài khoản đang là nhà cung cấp hoặc cộng tác viên thì không gán làm đại lý được, và ngược lại. Muốn đổi vai thì phải gỡ vai cũ trước — hãy liên hệ quản trị viên, đừng tạo tài khoản thứ hai bằng cùng số điện thoại (số điện thoại không cho trùng).

### 3. Khu đại lý gồm những gì

Sau khi đăng nhập, mở trang tài khoản của bạn. Trong thanh menu bên trái có một nhóm riêng tên "KHU ĐẠI LÝ" — chỉ tài khoản đại lý mới nhìn thấy nhóm này.

| Mục menu           | Đường dẫn                      | Dùng để làm gì                                               |
| ------------------ | ------------------------------ | ------------------------------------------------------------ |
| Bảng điều khiển    | `/tai-khoan/dai-ly/hang`       | Hạng, doanh số, quyền lợi từng dịch vụ.                      |
| Tuyến dưới của tôi | `/tai-khoan/dai-ly/tuyen-duoi` | Cây tuyến, đặt tỉ lệ chia cho cấp dưới.                      |
| Khách hàng         | `/tai-khoan/dai-ly/khach-hang` | Danh sách khách bạn đã giới thiệu và đơn của họ.             |
| Sổ hoa hồng        | `/tai-khoan/dai-ly/hoa-hong`   | Từng dòng tiền: đơn nào, cấp mấy, bao nhiêu, đã vào ví chưa. |
| Link giới thiệu    | `/tai-khoan/dai-ly/lien-ket`   | Tạo và quản lý link chia sẻ.                                 |
| Nạp tiền vào ví    | `/tai-khoan/dai-ly/nap-tien`   | Nạp tiền và xem lịch sử giao dịch ví.                        |

Menu thiếu mục nào đó?"Tuyến dưới của tôi" chỉ hiện khi sàn bật chế độ phân cấp. Nếu bạn không thấy mục này, sàn đang chạy đơn cấp — không phải tài khoản bạn bị lỗi.

### 4. Bảng điều khiển & hạng

Hạng (Đồng, Bạc, Vàng, Kim cương… tuỳ sàn đặt tên) quyết định NGÂN SÁCH mà sàn chi ra cho mỗi đơn. Mỗi hạng có một bảng mức riêng cho từng dịch vụ, và có thể khai riêng cho từng danh mục.

#### Nếu bạn là đại lý Cấp 1

* Trang hiện hạng hiện tại của bạn, doanh số toàn nhánh, và còn bao nhiêu nữa thì lên hạng kế tiếp.
* Bảng "Quyền lợi" liệt kê mức áp dụng cho từng dịch vụ. Mức có thể là PHẦN TRĂM hoặc SỐ TIỀN CỐ ĐỊNH.

#### Nếu bạn là đại lý Cấp 2 trở xuống

Bạn sẽ KHÔNG BAO GIỜ có hạng riêng — và đó không phải thiệt thòi

Chỉ đại lý Cấp 1 (người đứng đầu nhánh) mới mang hạng. Ngân sách của mỗi đơn trong nhánh do hạng của người đó quyết định, rồi chảy xuống theo tỉ lệ chia từng cấp. Trang bảng điều khiển của bạn vì vậy hiện: bạn đang ở cấp mấy, doanh số nhánh của bạn, số dư ví, tỉ lệ bạn chia cho cấp dưới, và hạng của đại lý gốc.

Đừng đề nghị quản trị viên "gán hạng" cho bạn — hệ thống chặn việc đó, vì gán hạng cho cấp 2+ sẽ tạo ra hai ngân sách chồng nhau trên cùng một đơn.

#### Mức áp dụng cho một đơn được chọn thế nào

1. Tìm mức khai riêng cho DANH MỤC của sản phẩm trước.
2. Sản phẩm thuộc nhiều danh mục cùng lúc → lấy mức TRẢ NHIỀU TIỀN NHẤT trên chính đơn đó (so bằng tiền, không so con số thô — 5% của đơn 10 triệu lớn hơn mức cố định 100.000đ).
3. Không có mức riêng theo danh mục → dùng mức mặc định của dịch vụ.
4. Không có cả hai → đơn đó không phát sinh quyền lợi nào.

Mức 0 và "không khai" là hai chuyện khác nhau Quản trị viên có thể khai mức 0 cho một danh mục để loại hẳn danh mục đó khỏi chương trình, dù dịch vụ mẹ vẫn có mức. Thấy một sản phẩm không sinh hoa hồng thì trước khi báo lỗi, hãy kiểm tra xem danh mục của nó có bị loại không.

### 5. Đưa khách vào tuyến

Bạn KHÔNG tự thêm được ai vào tuyến Không thêm được khách, cũng không thêm được đại lý cấp dưới. Đây là luật cố định của hệ thống: người mới phải TỰ hành động — tự bấm link của bạn, tự đăng ký, tự gõ mã của bạn. Không có màn hình nào cho phép bạn tạo tài khoản hộ người khác.

Chỉ có đúng hai đường, cả hai đều cần chính khách hàng hành động:

#### Đường 1 — Link giới thiệu

**Lấy link ở đâu**

* Ngay tại trang chi tiết dịch vụ: nút "Lấy link cho khách" nằm ngay dưới tên sản phẩm. Bấm là có link cho đúng trang đó.
* Hoặc vào trang "Link giới thiệu" trong khu đại lý, dán đường dẫn trang muốn chia sẻ rồi bấm tạo.

Một trang = một link Bấm nút nhiều lần trên cùng một trang sẽ trả lại ĐÚNG link cũ chứ không đẻ link mới. Cố ý như vậy để số lượt bấm không bị chia nhỏ ra nhiều link giống hệt nhau, và để bạn không nhầm link nào đã gửi cho khách.

**Link trông như thế nào**

Dạng ngắn: `demo.tourkitweb.vn/r/xxxxxxxx`

**Khi khách bấm vào link**

1. Lượt bấm được cộng vào bộ đếm của link.
2. Trình duyệt của khách ghi nhớ mã của bạn trong 30 ngày. Khách chưa mua ngay, vài hôm sau quay lại mua thì vẫn tính cho bạn.
3. Nếu khách đang đăng nhập và CHƯA có người giới thiệu, họ được gắn vào tuyến của bạn ngay lập tức và VĨNH VIỄN.
4. Khách được đưa tới đúng trang bạn đã chia sẻ.

**Hai loại link, đừng gửi nhầm**

| Loại link               | Dẫn tới                                           | Gửi cho ai                                                               |
| ----------------------- | ------------------------------------------------- | ------------------------------------------------------------------------ |
| **Link bán hàng**       | Trang tour / khách sạn / dịch vụ                  | Khách mua hàng. Người bấm vào mua ngay được, không phải đăng ký gì.      |
| **Link mời làm đại lý** | Trang đăng ký đại lý, ô mã đã điền sẵn số của bạn | Người muốn làm đại lý cấp dưới của bạn. Họ chỉ cần điền tên và mật khẩu. |

Đừng gửi link mời làm đại lý cho khách mua hàng: họ mở ra thấy trang đăng ký đại lý, không thấy sản phẩm, và thường sẽ đóng luôn.

**Xoá một link**

Khách đã được gắn vào tuyến trước đó VẪN LÀ của bạn — quan hệ đó vĩnh viễn, xoá link không gỡ được. Thứ mất đi là tương lai: ai bấm vào link cũ sau khi xoá sẽ bị đưa về trang chủ và không được gắn vào tuyến. Link đã dán lên Facebook, Zalo hay gửi qua tin nhắn thì bạn không thu về được, nên cân nhắc trước khi xoá.

#### Đường 2 — Khách gõ mã ở bước thanh toán

Ở bước thanh toán có ô "Mã giới thiệu". Khách gõ SỐ ĐIỆN THOẠI của bạn vào đó.

* Gõ được cả dạng 0912…, +84912… hay 84912… — hệ thống tự hiểu là cùng một số.
* Ô này dùng chung với mã cộng tác viên affiliate. Hệ thống tra số điện thoại đại lý trước, không khớp mới thử mã affiliate.
* Khách được báo ngay tại chỗ mã có hợp lệ không, không phải bấm gửi rồi mới biết.
* **Mã sai không bao giờ chặn đơn** — khách vẫn thanh toán bình thường, chỉ là không ai được ghi nhận.

**Mã KHÔNG áp dụng được trong các trường hợp sau**

| Tình huống                            | Hệ thống báo gì                                                                                                    |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Khách đã có người giới thiệu từ trước | "Bạn đã có người giới thiệu nên mã này không được áp dụng." — luật ai-đến-trước, người gắn đầu tiên giữ vĩnh viễn. |
| Khách gõ chính số của mình            | "Không thể tự giới thiệu chính mình."                                                                              |
| Người mua là cộng tác viên affiliate  | "Đã là Affiliate. Mã áp dụng không hợp lệ."                                                                        |
| Người mua là nhà cung cấp hoặc đại lý | Ô nhập bị ẩn hẳn, không hiện ra.                                                                                   |
| Đại lý được gõ mã đang bị khoá        | "Mã giới thiệu không hợp lệ." — giống hệt như gõ sai số.                                                           |

### 6. Tuyến dưới & tỉ lệ chia

Trang "Tuyến dưới của tôi" hiện toàn bộ nhánh bên dưới bạn: số cấp dưới trực tiếp, tổng số người toàn nhánh, doanh số nhánh và số đơn. Phần cây có thể gấp/mở từng nhánh; mỗi dòng ghi sẵn nhánh đó có bao nhiêu người bên dưới.

#### Đặt tỉ lệ chia cho cấp dưới

* Một tỉ lệ duy nhất, áp cho TẤT CẢ cấp dưới trực tiếp của bạn. Không đặt riêng từng người được — cố ý như vậy để bảng tính tiền còn đối soát nổi.
* Trần sàn quy định: 70%. Gõ cao hơn thì hệ thống tự hạ về trần và báo cho bạn biết, không im lặng.
* Đại lý mới vào cây được đặt sẵn 50%.
* Đổi tỉ lệ chỉ ảnh hưởng ĐƠN PHÁT SINH TỪ LÚC ĐỔI. Đơn cũ đã ghi sổ không tính lại.

Tỉ lệ cao hay thấp thì lợi hơn? Tỉ lệ cao → cấp dưới nhận nhiều hơn trên mỗi đơn họ bán, nên họ có động lực bán và tuyển thêm; nhưng phần bạn giữ trên mỗi đơn của họ mỏng đi. Tỉ lệ thấp thì ngược lại. Đây là quyết định kinh doanh của bạn, hệ thống không khuyến nghị con số nào.

#### Ví dụ với một đơn có ngân sách 1.000.000đ

* Đơn do CHÍNH BẠN mang về → bạn nhận trọn 1.000.000đ.
* Đơn do CẤP DƯỚI TRỰC TIẾP mang về (bạn chia 50%) → họ nhận 500.000đ, bạn nhận 500.000đ.

Bạn chỉ thấy tỉ lệ của cấp dưới TRỰC TIẾP Tỉ lệ mà cấp dưới của bạn chia cho tuyến dưới của họ là thoả thuận riêng giữa họ với nhau, không hiện cho bạn. Nó cũng không quyết định đồng nào chảy vào túi bạn.

#### Cấp dưới bị khoá hoặc nghỉ

Node của họ vẫn nằm nguyên trong cây, cấu trúc không đổi. Phần tiền của riêng họ bị giữ lại (ghi sổ là "Không được tính"), còn bạn và mọi người còn lại nhận đúng như cũ — không ai được thêm, không ai bị bớt.

### 7. Tiền được tính thế nào

#### Bước 1 — Ai là "người bán" của đơn

1. Nếu người đặt đơn CHÍNH LÀ một tài khoản đại lý → người đó là người bán. Đại lý đặt hộ khách bằng tài khoản của mình là nghiệp vụ bình thường, không phải gian lận.
2. Ngược lại → lấy người đã giới thiệu khách đó (quan hệ gắn bằng link hoặc bằng mã ở checkout).
3. Không ra ai → đơn không phát sinh hoa hồng.

#### Bước 2 — Ngân sách của đơn

Ngân sách = mức của hạng (hạng của đại lý Cấp 1 đứng đầu nhánh) áp lên GIÁ TRƯỚC PHÍ của đơn, theo đúng dịch vụ và danh mục của sản phẩm.

* Giá trước phí là giá đã trừ khuyến mãi / mã giảm giá, chưa cộng các loại phí dịch vụ.
* Ngân sách bằng 0 → không sinh dòng nào. Thường là do hạng chưa khai mức cho dịch vụ đó, hoặc hạng đang bị tắt.

#### Bước 3 — Chia theo chuỗi cấp

Ngân sách chảy từ Cấp 1 xuống tới người bán. Mỗi cấp đẩy xuống một phần theo tỉ lệ mình đã đặt, và GIỮ LẠI phần chênh giữa cái chảy vào mình với cái mình đẩy tiếp xuống.

| Người bán ở cấp | Cấp 1 nhận | Cấp 2 nhận | Cấp 3 nhận | Tổng chi    |
| --------------- | ---------- | ---------- | ---------- | ----------- |
| Cấp 1           | 100.000    | 0          | 0          | **100.000** |
| Cấp 2           | 40.000     | 60.000     | 0          | **100.000** |
| Cấp 3           | 40.000     | 30.000     | 30.000     | **100.000** |

(Ví dụ với ngân sách 100.000đ, mỗi cấp chia 60% xuống dưới.) Dù chuỗi dài bao nhiêu, tổng chi không bao giờ vượt ngân sách của đơn.

Đồng lẻ ở lại người chiaMọi con số đều làm tròn XUỐNG về đồng nguyên. Phần lẻ nằm lại ở cấp trên gần nhất — người đẩy tiền xuống. Vì vậy con số bạn nhận đôi khi lẻ hơn cấp dưới một đồng, đó là đúng quy tắc chứ không phải sai sót.

#### Bước 4 — Khi nào tiền được ghi sổ

| Đơn chuyển sang                        | Hệ thống làm gì                                                           |
| -------------------------------------- | ------------------------------------------------------------------------- |
| Đã thanh toán / Đã xác nhận / Hoàn tất | Ghi sổ hoa hồng cho cả chuỗi. Gọi lại bao nhiêu lần cũng chỉ ghi một lần. |
| Đã huỷ                                 | Thu hồi toàn bộ. Tiền đã vào ví bị trừ ra.                                |
| Bị xoá                                 | Thu hồi như đơn huỷ.                                                      |
| Nháp / chờ thanh toán / đang xử lý     | Chưa sinh gì cả.                                                          |

Số dư ví có thể ÂMNếu bạn đã rút hoặc đã tiêu hết tiền hoa hồng rồi đơn mới bị huỷ, phần thu hồi làm số dư âm — đó là khoản nợ với sàn. Nó tự trừ dần vào hoa hồng bạn kiếm được sau đó. Trong lúc số dư còn âm, bạn không thanh toán đơn bằng ví được.

#### Bước 5 — Từ sổ vào ví

Sàn giữ tiền 5 ngày, tính từ NGÀY KẾT THÚC DỊCH VỤ của đơn (ngày về của tour, ngày trả phòng…). Trong thời gian đó dòng tiền hiện trạng thái "Đang giữ" — đã chắc chắn là của bạn nhưng chưa dùng được. Hết hạn, hệ thống tự cộng vào ví mỗi đêm.

### 8. Sổ hoa hồng

Mỗi đơn sinh ra MỘT DÒNG cho mỗi người trong chuỗi — kể cả người không được nhận. Dòng không được nhận vẫn ghi lại kèm lý do, để bạn luôn đối chiếu được và không bao giờ phải đoán.

| Trạng thái      | Nghĩa là                                                                    |
| --------------- | --------------------------------------------------------------------------- |
| Đã vào ví       | Tiền đã cộng vào số dư, dùng được.                                          |
| Đang giữ        | Đã ghi sổ, chờ hết thời gian giữ mới vào ví.                                |
| Không được tính | Bạn nằm trong chuỗi nhưng không đủ điều kiện nhận. Cột lý do ghi rõ vì sao. |
| Đã thu hồi      | Đơn bị huỷ hoặc bị xoá sau khi đã trả tiền.                                 |

#### Bốn lý do khiến một dòng "Không được tính"

1. **Tài khoản đại lý đang bị khoá** — quản trị viên đã tạm ngừng quyền nhận hoa hồng của tài khoản đó. Vẫn đăng nhập và mua hàng bình thường.
2. **Tài khoản đang bị khoá** — tài khoản bị khoá đăng nhập.
3. **Tài khoản đã bị xoá**
4. **Tài khoản không còn là đại lý** — đã bị gỡ khỏi danh sách đại lý.

Phần tiền của dòng bị loại được SÀN GIỮ LẠI, không dồn lên cấp trên. Điều này cố ý: nếu dồn lên thì cấp trên sẽ có động cơ khoá cấp dưới để ăn trọn ngân sách.

### 9. Khách hàng của tôi

Danh sách những người đã gắn vào tuyến của bạn (bằng link hoặc bằng mã). Tìm được theo tên, email, số điện thoại. Bấm vào từng khách để xem toàn bộ đơn của họ kèm tổng số đơn, tổng giá trị và tổng hoa hồng bạn đã nhận từ khách đó.

Danh sách này chỉ gồm KHÁCH, không gồm đại lý cấp dướiCấp dưới của bạn nằm ở trang "Tuyến dưới của tôi". Hai danh sách khác nhau và không trùng nhau.

### 10. Ví, nạp tiền & thanh toán

Mỗi tài khoản đại lý có một ví. Ví nhận hoa hồng, nhận tiền bạn nạp vào, và dùng để thanh toán đơn ngay trên website qua phương thức "Ví đại lý".

#### Nạp tiền

1. **Tạo yêu cầu.** Vào "Nạp tiền vào ví" → nhập số tiền (tối thiểu 1.000đ).
2. **Chuyển khoản.** Màn hình hiện thông tin ngân hàng của sàn, mã QR VietQR và MỘT MÃ CHUYỂN KHOẢN riêng cho yêu cầu này.
3. **Ghi đúng mã vào nội dung chuyển khoản.** Quét QR thì nội dung đã điền sẵn. Chuyển tay mà ghi sai nội dung thì kế toán không đối chiếu được, tiền vào chậm.
4. **Bấm "Tôi đã chuyển khoản".** Việc này chỉ báo cho sàn biết để kiểm tra — chưa cộng tiền.
5. **Chờ xác nhận.** Quản trị viên kiểm tra sao kê rồi xác nhận, tiền mới vào ví.

| Trạng thái giao dịch | Nghĩa là                                                                         |
| -------------------- | -------------------------------------------------------------------------------- |
| Chờ xử lý            | Đã tạo yêu cầu, sàn chưa xác nhận. Tiền CHƯA vào ví.                             |
| Đã xác nhận          | Tiền đã cộng vào số dư.                                                          |
| Đã huỷ               | Sàn từ chối yêu cầu (thường do không tìm thấy giao dịch chuyển khoản tương ứng). |

#### Thanh toán bằng ví

* Ở bước chọn phương thức thanh toán sẽ có "Ví đại lý", kèm số dư hiện tại. Phương thức này chỉ hiện với tài khoản đại lý và chỉ khi sàn đã bật nó.
* Số dư không đủ → hệ thống báo và không trừ gì cả. Nạp thêm rồi thanh toán lại.
* Trả một phần thì đơn ở trạng thái thanh toán một phần, phần còn lại trả sau.

Lịch sử giao dịch ví ≠ sổ hoa hồngVí còn nhận tiền nạp, tiền hoàn, tiền quản trị viên cộng tay. Muốn đối chiếu riêng phần hoa hồng thì xem trang "Sổ hoa hồng"; số dư ví không bao giờ bằng đúng tổng hoa hồng.

### 11. Lên hạng và xuống hạng

Hệ thống tự xét lại hạng MỖI NGÀY MỘT LẦN, dựa trên doanh số toàn nhánh.

#### Doanh số toàn nhánh gồm những gì

* Đơn do chính bạn đặt bằng tài khoản của mình.
* Đơn của mọi khách bạn đã giới thiệu.
* Toàn bộ đơn của mọi cấp dưới trong nhánh, ở mọi độ sâu, kể cả khách của họ.
* Chỉ tính đơn đã thanh toán / đã xác nhận / đã hoàn tất. Đơn huỷ và đơn chưa trả tiền không tính.

Hạng có thể XUỐNG, không chỉ lênDoanh số nhánh rơi xuống dưới ngưỡng của hạng hiện tại thì hệ thống tự hạ hạng, và quyền lợi của cả nhánh giảm theo. Doanh số tính trên toàn bộ lịch sử đơn, không theo tháng.Chỉ đại lý Cấp 1 mới được xét hạng. Cấp 2 trở xuống không có hạng nên lệnh này bỏ qua họ — điều đó không có nghĩa họ bị bỏ quên: doanh số của họ vẫn được cộng vào nhánh và vẫn kéo hạng của Cấp 1 lên.

### 12. Tình huống thường gặp

Mở tất cảThu gọn tất cả

<details>

<summary>Khách của tôi đã mua rồi mà tôi không thấy hoa hồng đâu</summary>

Kiểm tra lần lượt, gần như luôn rơi vào một trong các mục sau:

1. Khách có trong trang "Khách hàng của tôi" không? Không có nghĩa là họ chưa từng được gắn vào tuyến của bạn.
2. Đơn đã ở trạng thái đã thanh toán / đã xác nhận / hoàn tất chưa? Đơn còn chờ thanh toán thì chưa sinh gì.
3. Mở "Sổ hoa hồng" — có dòng nào cho đơn đó không? Có dòng "Không được tính" thì đọc cột lý do.
4. Có dòng "Đang giữ"? Vậy là đã tính đúng, chỉ chưa tới hạn vào ví.
5. Hạng của bạn có khai mức cho DỊCH VỤ đó không (tour khác khách sạn khác vé máy bay)? Có mức riêng cho danh mục của sản phẩm và mức đó bằng 0 không?
6. Đơn có được giảm giá sâu bằng mã khuyến mãi không? Ngân sách tính trên giá sau giảm.
7. Không mục nào đúng → liên hệ quản trị viên kèm MÃ ĐƠN. Họ có lệnh đối soát tra ra ngay đơn đó rơi vào đâu.

</details>

<details>

<summary>Khách bấm link của tôi nhưng ở bước thanh toán lại gõ mã của người khác</summary>

Người gắn ĐẦU TIÊN thắng và giữ vĩnh viễn. Khách bấm link của bạn khi đang đăng nhập thì họ đã thuộc tuyến của bạn ngay lúc đó, mã gõ sau bị từ chối với thông báo "Bạn đã có người giới thiệu". Ngược lại, nếu lúc bấm link họ chưa đăng nhập và sau đó gõ mã người khác thì người kia thắng.

</details>

<details>

<summary>Khách mua xong rồi tôi mới gửi link — có tính được không?</summary>

Không. Quan hệ giới thiệu chỉ áp cho đơn phát sinh TỪ SAU khi gắn. Đơn đã đặt trước đó không được tính lại. Từ đơn thứ hai trở đi thì tính bình thường, không cần khách gõ lại mã.

</details>

<details>

<summary>Khách bấm link nhưng chưa có tài khoản</summary>

Vẫn được. Trình duyệt của họ nhớ mã của bạn 30 ngày; khi họ đăng ký tài khoản trong khoảng thời gian đó, hệ thống tự gắn họ vào tuyến của bạn. Nếu họ mua với tư cách khách vãng lai và có tạo tài khoản ở bước thanh toán thì cũng vậy.

Rủi ro duy nhất: họ xoá cookie, đổi máy, hoặc dùng chế độ ẩn danh. Khi đó nhắc họ gõ số điện thoại của bạn vào ô "Mã giới thiệu" là chắc chắn nhất.

</details>

<details>

<summary>Tôi đổi số điện thoại thì mã giới thiệu có đổi không?</summary>

Có — mã chính là số điện thoại trên tài khoản, đổi số là mã đổi theo và khách phải gõ số mới. Riêng LINK giới thiệu thì không ảnh hưởng: link chạy bằng mã riêng của nó, mọi link đã phát tán vẫn hoạt động bình thường.

</details>

<details>

<summary>Cấp dưới của tôi tự mua hàng cho chính họ — tôi có được chia không?</summary>

Có. Đơn do một đại lý đặt bằng tài khoản của chính họ được coi là đơn họ bán, nên cả tuyến trên vẫn được chia đúng theo công thức.

</details>

<details>

<summary>Tôi tuyển được người mới nhưng họ không hiện trong tuyến dưới</summary>

Người mới chỉ vào cây khi họ đăng ký tại TRANG ĐĂNG KÝ ĐẠI LÝ và điền đúng số điện thoại của bạn vào ô mã giới thiệu. Nếu họ đăng ký tài khoản khách thường rồi mới xin lên đại lý thì quản trị viên duyệt sẽ tạo họ thành Cấp 1 ĐỘC LẬP — không thuộc nhánh của bạn, và không có cách nào chuyển sang sau đó mà không đổi tiền của cả nhánh. Hãy luôn gửi họ link mời làm đại lý.

</details>

<details>

<summary>Một người trong nhánh bị khoá thì tuyến dưới họ có mất tiền không?</summary>

Không. Node bị khoá vẫn nằm nguyên chỗ cũ trong cây, chuỗi không ngắn đi. Chỉ phần của riêng người bị khoá bị giữ lại; mọi người khác nhận đúng số tiền như trước.

</details>

<details>

<summary>Tôi nghỉ làm đại lý thì chuyện gì xảy ra?</summary>

* Tài khoản vẫn dùng được như khách thường, đơn cũ giữ nguyên.
* Bạn không nhận hoa hồng mới nữa; các dòng đang giữ có thể không được cộng vào ví.
* Tuyến dưới của bạn KHÔNG bị xoá và cũng không bị đổi tiền — họ vẫn nhận đúng như cũ, chỉ phần của bạn là sàn giữ lại.
* Số dư ví còn lại: liên hệ sàn để tất toán.

</details>

<details>

<summary>Vé máy bay có tính hoa hồng không?</summary>

Có, nếu hạng của bạn có khai mức cho dịch vụ "Vé máy bay". Riêng vé máy bay KHÔNG có nút "Lấy link cho khách" ở trang đặt vé — muốn gắn khách thì dùng link tới trang khác, hoặc để khách gõ mã ở bước thanh toán.

</details>

<details>

<summary>Trước đây tôi đặt được giá giảm, giờ hết giảm</summary>

Sàn đã bật chế độ phân cấp. Chiết khấu trực tiếp bị cắt cho toàn bộ đại lý, quyền lợi chuyển sang đường hoa hồng vào ví. Bạn không mất quyền lợi, chỉ đổi cách nhận: trước đây trừ thẳng vào giá, giờ nhận sau khi đơn hoàn tất.

</details>

<details>

<summary>Ví tôi đang bị âm</summary>

Số âm là khoản nợ, phát sinh khi đơn bị huỷ sau lúc bạn đã dùng hết tiền hoa hồng của đơn đó. Hệ thống không xoá nợ mà tự trừ dần vào hoa hồng bạn kiếm được sau này. Bạn cũng có thể nạp tiền vào ví để đưa số dư về dương ngay.

</details>

<details>

<summary>Tôi vừa muốn làm đại lý vừa muốn làm cộng tác viên affiliate</summary>

Không được — ba loại tài khoản (đại lý, nhà cung cấp, cộng tác viên) loại trừ nhau, mỗi tài khoản chỉ mang một loại. Đây là ràng buộc cố ý: hai chương trình có hai cách tính tiền riêng, nếu để chồng nhau thì một đơn sẽ trả tiền hai lần từ hai ngân sách khác nhau.

</details>

<details>

<summary>Bấm vào mục nào trong khu đại lý cũng bị đá về trang hồ sơ</summary>

Tài khoản đang không được nhận diện là đại lý. Thường là do hồ sơ chưa được duyệt, hoặc bạn vừa bị gỡ khỏi danh sách đại lý. Liên hệ quản trị viên để kiểm tra.

</details>
